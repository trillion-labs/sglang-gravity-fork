...
def test_ltx23_refinement_stage_noises_in_unpacked_latent_space(monkeypatch):
    captured = {}

    def fake_forward(self, batch, server_args):
        captured["packed_video_latents"] = batch.latents.clone()
        captured["packed_audio_latents"] = batch.audio_latents.clone()
        return batch
    ...
    unpacked_video = torch.tensor(
        [[[[[0.0, 1.0]], [[2.0, 3.0]]]]], dtype=torch.float32
    )
    unpacked_audio = torch.tensor(
        [[[[10.0, 11.0], [12.0, 13.0]]]], dtype=torch.float32
    )

    class FakePipelineConfig:
        def _unpad_and_unpack_latents(
            self, latents, audio_latents, batch, vae, audio_vae
        ):
            return unpacked_video.clone(), unpacked_audio.clone()

        def maybe_pack_latents(self, latents, batch_size, batch):
            return latents.reshape(1, 2, 2)

        def maybe_pack_audio_latents(self, latents, batch_size, batch):
            return latents.reshape(1, 2, 2)

    req_generator = torch.Generator(device="cpu").manual_seed(123)
    expected_generator = torch.Generator(device="cpu").manual_seed(123)
    req = Req(
        sampling_params=SamplingParams(seed=123),
        prompt="prompt",
        latents=torch.zeros((1, 2, 2), dtype=torch.float32),
        audio_latents=torch.zeros((1, 2, 2), dtype=torch.float32),
        generator=[req_generator],
        timesteps=torch.tensor([42.0], dtype=torch.float32),
        num_inference_steps=1,
    )

    result = stage.forward(req, SimpleNamespace(pipeline_config=FakePipelineConfig()))

    assert result is req
    noise_scale = 0.9
    expected_video_noise = torch.randn(
        unpacked_video.shape,
        generator=expected_generator,
        dtype=unpacked_video.dtype,
    )
    expected_audio_noise = torch.randn(
        unpacked_audio.shape,
        generator=expected_generator,
        dtype=unpacked_audio.dtype,
    )
    expected_video = (
        expected_video_noise * noise_scale + unpacked_video * (1 - noise_scale)
    ).reshape(1, 2, 2)
    expected_audio = (
        expected_audio_noise * noise_scale + unpacked_audio * (1 - noise_scale)
    ).reshape(1, 2, 2)

    assert torch.allclose(captured["packed_video_latents"], expected_video)
    assert torch.allclose(captured["packed_audio_latents"], expected_audio)
